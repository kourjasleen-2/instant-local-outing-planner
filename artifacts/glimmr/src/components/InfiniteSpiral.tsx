...
export interface InfiniteSpiralProps {
  ...
  grayscale?: number;
  className?: string;
  renderItem?: (item: SpiralItem, index: number) => ReactNode;
  radiusCapFactor?: number;
  maxFit?: number;
}
...
const InfiniteSpiral = ({
  ...
  const normalizedItems = useMemo<SpiralItem[]>(
    () =>
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const fit = Math.min(maxFit, width / (cardWidth * 2.8), height / (cardHeight * 2.35));
      const responsiveRadius = Math.min(radius, Math.max(72, width * radiusCapFactor)) * fit;
        ...
        {normalizedItems.map((item, index) => {
          ...
          return (
            <Card
              ...
              className={`infinite-spiral__item ${renderItem ? 'infinite-spiral__item--custom' : ''}`.trim()}
              style={{ width: cardWidth, height: cardHeight, borderRadius: cardRadius }}
              href={item.href}
              target={item.target}
              rel={item.target === '_blank' ? 'noreferrer' : undefined}
              role="listitem"
              aria-label={item.label ?? item.alt ?? item.name ?? item.id ?? `Card ${index + 1}`}
            >
              {renderItem ? (
